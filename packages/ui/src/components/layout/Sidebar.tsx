...
import { useCapabilities } from "@/platform/host";
...
export function Sidebar() {
  ...
  const { auth, classroom } = useCapabilities();
  const currentChoice = getStoredChoice();

  const navItems = [
    ...(auth ? [{ to: "/", labelKey: "nav.dashboard", icon: Home }] : []),
    ...(auth ? [{ to: "/videos", labelKey: "nav.videos", icon: Video }] : []),
    ...(classroom ? [{ to: "/classroom", labelKey: "nav.classroom", icon: Radio }] : []),
  ] as const;
